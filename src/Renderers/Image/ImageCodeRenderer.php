...
class ImageCodeRenderer extends CodeRenderer
{
        ...
        for ($y = 0; $y < $this->frame->size; $y++) {
            for ($x = 0; $x < $this->frame->size; $x++) {
                ...
                switch ($this->frame->pixels[$y][$x]) {
                    case CodeFrame::$symbolBorder :
                        $themeElement = $borderThemeElement;
                        break;
                    case CodeFrame::$symbolPixelOn :
                        $themeElement = 'pixelOn';
                        break;
                    case CodeFrame::$symbolPixelOff :
                        $themeElement = 'pixelOff';
                        break;
                    default:
                        break;
                }
                ...
                \imagecopy($image, $this->config->$themeElement, $x * $this->config->pixelPerPoint,
                    $y * $this->config->pixelPerPoint, 0, 0, $this->config->pixelPerPoint,
                    $this->config->pixelPerPoint);
            }
        }
    }
    ...
    private function renderSpecialElement(&$image, $elements, $themeElement, $imagePointSize)
    {
        foreach ($elements as $element) {
            \imagecopy($image, $this->config->$themeElement, $element[0] * $this->config->pixelPerPoint,
                $element[1] * $this->config->pixelPerPoint, 0, 0, $imagePointSize * $this->config->pixelPerPoint,
                $imagePointSize * $this->config->pixelPerPoint);
        }
    }
}
